...
function NeosisChatInner() {
  ...
  return (
    <div className="flex h-screen bg-[#111313] transition-colors duration-300 font-sans relative overflow-hidden">
      <AnimatePresence>
        {!hasAcceptedTC && (
          <motion.div 
          ...
          >
            <motion.div 
              ...
              className="bg-[#1a1f1d] w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden border border-[#323d38]"
            >
              <div className="p-8 text-center bg-gradient-to-b from-[#0fa384]/10 to-transparent">
                ...
                <h2 className="text-2xl font-bold text-white mb-2 font-display">Welcome to Neosis</h2>
                <p className="text-sm text-gray-400">Please review our security and privacy guidelines.</p>
              </div>
              
              <div className="px-8 py-2 max-h-60 overflow-y-auto custom-scrollbar text-sm text-gray-300 space-y-4">
                <div className="flex gap-3 items-start">
                  ...
                  <div>
                    ...
                    <p className="mt-1 leading-relaxed text-gray-400">All video and audio calls are secured via WebRTC DTLS-SRTP encryption.</p>
                  </div>
                </div>
                <div className="flex gap-3 items-start">
                  ...
                  <div>
                    ...
                    <p className="mt-1 leading-relaxed text-gray-400">We do not sell your personal data. Your chat history is stored securely.</p>
                  </div>
                </div>
                <div className="flex gap-3 items-start">
                  ...
                  <div>
                    ...
                    <p className="mt-1 leading-relaxed text-gray-400">Harassment, spam, and illegal activities are strictly prohibited.</p>
                  </div>
                </div>
              </div>

              <div className="p-8 bg-[#151817] mt-4 border-t border-[#232a28] flex flex-col gap-3">
                <motion.button 
      ...
      <motion.div variants={pageTransition} initial="hidden" animate="show" className={`w-full h-full flex z-10 ${!hasAcceptedTC ? 'pointer-events-none blur-sm' : ''}`}>
        ...
        <div className={`${activeChat ? 'hidden md:flex' : 'flex'} w-full md:w-[350px] bg-[#1a1f1d] flex-col flex-shrink-0 z-20 border-r border-[#232a28] transition-colors duration-300`}>
          <div className="p-4 flex justify-between items-center transition-colors duration-300 border-b border-[#232a28]">
            <div className="flex items-center gap-3">
              ...
              <div><h2 className="text-base font-bold text-white tracking-wide font-display">NEOSIS</h2></div>
            </div>
            <div className="flex items-center gap-1">
              
              <div className="relative">
                <motion.button aria-label="Notifications" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => setShowNotifications(!showNotifications)} className="p-2 text-gray-400 hover:text-white hover:bg-[#232a28] rounded-lg transition-colors">
                  <Bell size={18} />{pendingRequests.length > 0 && <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-rose-500 rounded-full border-2 border-[#1a1f1d]"></motion.span>}
                </motion.button>
                <AnimatePresence>
                  {showNotifications && (
                    <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={{ type: "spring", stiffness: 300, damping: 25 }} className="absolute top-12 right-0 w-80 bg-[#151817] rounded-xl shadow-2xl border border-[#323d38] z-50 overflow-hidden">
                      <div className="p-4 border-b border-[#232a28] text-sm font-bold flex justify-between items-center text-white"><span>Friend Requests</span><span className="bg-[#0fa384]/10 text-[#0fa384] text-xs px-2.5 py-1 rounded-full">{pendingRequests.length}</span></div>
                      {pendingRequests.length === 0 ? <div className="p-8 text-sm text-gray-400 text-center">No pending requests</div> : (
                        <div className="max-h-64 overflow-y-auto custom-scrollbar">
                          {pendingRequests.map(req => (
                            <div key={req.id} className="p-4 border-b border-[#232a28]/50 flex justify-between items-center hover:bg-[#1a1f1d] transition">
                              <span className="text-sm font-medium text-gray-200 truncate w-2/3">{formatName(req.senderEmail)}</span>
              ...
              <div className="relative">
                <motion.button aria-label="Settings" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => setShowSettingsMenu(!showSettingsMenu)} className="p-2 text-gray-400 hover:text-white hover:bg-[#232a28] rounded-lg transition-colors">
                  <Settings size={18} />
                </motion.button>
                <AnimatePresence>
                  {showSettingsMenu && (
                    <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={{ type: "spring", stiffness: 300, damping: 25 }} className="absolute right-0 top-12 w-56 bg-[#151817] rounded-xl shadow-2xl border border-[#323d38] z-50 overflow-hidden">
                      <div className="p-4 border-b border-[#232a28] text-sm font-bold text-white">Settings</div>
                      <div className="flex flex-col text-sm text-gray-300">
                        <button onClick={() => { setShowSettingsMenu(false); showToast("Account preferences coming soon", "info"); }} className="flex items-center gap-3 px-4 py-3 hover:bg-[#1a1f1d] transition w-full text-left"><UserCog size={16} /> Account</button>
                        <button onClick={() => { setShowSettingsMenu(false); showToast("Privacy settings coming soon", "info"); }} className="flex items-center gap-3 px-4 py-3 hover:bg-[#1a1f1d] transition w-full text-left"><Shield size={16} /> Privacy</button>
                        <div className="h-px bg-[#232a28] w-full"></div>
                        <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-3 hover:bg-[#232a28] text-rose-500 transition w-full text-left font-semibold"><LogOut size={16} /> Log Out</button>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
          ...
          <div className="px-4 py-4 border-b border-[#232a28]">
            <form onSubmit={handleSendRequest} className="relative flex items-center">
              <Search size={18} className="absolute left-3 text-[#0fa384]" />
              <input type="email" value={addEmailInput} onChange={(e) => setAddEmailInput(e.target.value)} placeholder="Search contacts..." className="w-full bg-[#111313] text-white placeholder-gray-500 rounded-lg pl-10 pr-10 py-2.5 text-sm outline-none focus:border-[#0fa384] transition-colors border border-[#323d38] shadow-none" required />
              <button aria-label="Add Contact" type="submit" className="absolute right-2 text-[#0fa384] p-1 hover:bg-[#232a28] rounded-md transition-colors"><UserPlus size={18}/></button>
            </form>
          </div>
          ...
          <motion.div variants={listVariants} initial="hidden" animate="show" className="flex-1 overflow-y-auto custom-scrollbar contain-content">
            {friends.map(friend => {
              ...
              return (
                <motion.div variants={itemVariants} key={friend} onClick={() => handleOpenChat(friend)} className={`relative flex items-center gap-3.5 p-4 cursor-pointer transition-all duration-150 ${isActive ? 'bg-[#1f2422]' : 'hover:bg-[#151817]'}`}>
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-[#0fa384] rounded-full border-2 border-[#1a1f1d] transition-colors duration-300"></div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-baseline mb-0.5"><div className="font-medium text-gray-100 truncate text-[15px]">{fName}</div></div>
                    <div className="flex justify-between items-center">
                      <div className="text-xs text-gray-400 truncate">Tap to view conversation...</div>
            ...
            {friends.length === 0 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-10 flex flex-col items-center justify-center text-gray-500 text-center px-6">
                <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 3, ease: "easeInOut" }} className="w-16 h-16 bg-[#151817] shadow-none rounded-2xl flex items-center justify-center mb-4 transition-colors"><UserPlus size={24} className="text-[#0fa384]" /></motion.div>
                <h3 className="text-gray-200 font-medium mb-1 font-display">No chats yet</h3>
                <p className="text-xs leading-relaxed text-gray-500">Search for a friend's email above to start a secure conversation.</p>
              </motion.div>
            )}
          </motion.div>
        </div>

        <div className={`${!activeChat ? 'hidden md:flex' : 'flex'} flex-1 flex-col bg-[#111313] relative transition-colors duration-300`}>
          {!activeChat ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex-1 flex flex-col items-center justify-center relative z-10 text-gray-400">
              <motion.div animate={{ y: [0, -15, 0] }} transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }} className="w-24 h-24 bg-[#1a1f1d] rounded-full flex items-center justify-center mb-6 shadow-inner border border-[#232a28]"><MessageSquare size={40} className="text-[#0fa384]" /></motion.div>
              <h2 className="text-2xl font-bold text-white mb-2 font-display">Neosis Web</h2>
            ...
            <div className="flex flex-col h-full relative z-10">
              <div className="px-6 py-4 bg-[#1a1f1d] border-b border-[#232a28] flex items-center justify-between z-20 transition-colors">
                <div className="flex items-center gap-4">
                  <motion.button aria-label="Go Back" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => setActiveChat(null)} className="md:hidden text-gray-400 hover:text-white transition"><ArrowLeft size={24} /></motion.button>
                  <div className="relative">
                    ...
                    <div className="absolute bottom-0 right-0 w-3 h-3 bg-[#0fa384] rounded-full border-2 border-[#1a1f1d] transition-colors"></div>
                  </div>
                  <div>
                    <div className="font-bold text-white text-[16px]">{formatName(activeChat)}</div>
                ...
                <div className="flex items-center gap-2 text-gray-400">
                  <motion.button aria-label="Audio Call" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => handleStartCall(false)} className="p-2 rounded-lg hover:text-white hover:bg-[#202523] transition-colors"><Phone size={20}/></motion.button>
                  <motion.button aria-label="Video Call" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => handleStartCall(true)} className="p-2 rounded-lg hover:text-white hover:bg-[#202523] transition-colors"><Video size={22}/></motion.button>
                  <div className="w-px h-6 bg-[#323d38] mx-2 transition-colors"></div>
                  <motion.button aria-label="Search Chat" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => { setIsSearching(!isSearching); setSearchQuery(''); }} className={`p-2 rounded-lg hover:text-white hover:bg-[#202523] transition-colors ${isSearching ? 'text-[#0fa384] bg-[#0fa384]/10' : ''}`}><Search size={20}/></motion.button>
                  <div className="relative">
                    <motion.button aria-label="Menu" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => setShowMoreMenu(!showMoreMenu)} className="p-2 rounded-lg hover:text-white hover:bg-[#202523] transition-colors"><MoreVertical size={20}/></motion.button>
                    <AnimatePresence>
                      {showMoreMenu && (
                        <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={{ type: "spring", stiffness: 300, damping: 25 }} className="absolute right-0 top-10 w-48 bg-[#151817] rounded-xl shadow-xl border border-[#323d38] overflow-hidden z-50">
                          <div className="flex flex-col text-sm text-gray-200">
                            <button onClick={() => showToast("Contact profiles coming soon", "info")} className="flex items-center gap-3 px-4 py-3 hover:bg-[#1a1f1d] transition w-full text-left"><User size={16} /> Contact Info</button>
                            <button onClick={() => { setMessages([]); setShowMoreMenu(false); showToast("Local chat view cleared"); }} className="flex items-center gap-3 px-4 py-3 hover:bg-[#1a1f1d] transition w-full text-left"><Trash2 size={16} /> Clear Local Chat</button>
                          </div>
                        </motion.div>
                      )}
                    </AnimatePresence>
              ...
              <AnimatePresence>
                {isSearching && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="bg-[#1a1f1d] border-b border-[#232a28] overflow-hidden z-10 transition-colors">
                    <div className="px-6 py-3 flex items-center gap-3">
                      <Search size={16} className="text-[#0fa384]" />
                      <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search and filter this chat..." className="flex-1 bg-transparent text-sm outline-none text-white placeholder-gray-500" autoFocus />
              ...
              <div className="flex-1 p-6 overflow-y-auto custom-scrollbar space-y-5 bg-[#111313] transition-colors">
                      ...
                      return (
                        <motion.div layout variants={messageVariants} initial="hidden" animate="show" key={messageKey} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                          <div className={`flex flex-col max-w-[85%] md:max-w-[70%] ${isMe ? 'items-end' : 'items-start'}`}>
                            <div className={`px-4 py-2.5 text-[15px] leading-relaxed shadow-sm break-words ${isMe ? 'bg-[#0fa384] text-white rounded-2xl rounded-br-sm' : 'bg-[#232a28] text-gray-200 rounded-2xl rounded-tl-sm transition-colors'}`}>
                              {isSearching && searchQuery ? highlightText(rawContent, searchQuery) : rawContent}
                            </div>
                            <span className="text-[10px] text-gray-500 mt-1 font-medium px-1 flex items-center gap-1 font-mono">
                              {msg.timestamp || 'Just now'} 
                              {isMe && (isPending ? <Check size={12} className="text-gray-600" /> : <CheckCheck size={14} className="text-[#0fa384]" />)}
                            </span>
                          </div>
                        </motion.div>
                      );
                    })}
                    {isRemoteTyping && <motion.div layout initial={{ opacity: 0, y: 20, scale: 0.8 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ type: "spring" }} className="flex justify-start"><div className="bg-[#232a28] transition-colors rounded-2xl rounded-tl-sm px-4 py-3.5 flex items-center gap-1.5 w-16 shadow-sm"><motion.div animate={{ y: [0, -4, 0] }} transition={{ duration: 0.6, repeat: Infinity, delay: 0 }} className="w-1.5 h-1.5 bg-[#0fa384] rounded-full" /><motion.div animate={{ y: [0, -4, 0] }} transition={{ duration: 0.6, repeat: Infinity, delay: 0.2 }} className="w-1.5 h-1.5 bg-[#0fa384] rounded-full" /><motion.div animate={{ y: [0, -4, 0] }} transition={{ duration: 0.6, repeat: Infinity, delay: 0.4 }} className="w-1.5 h-1.5 bg-[#0fa384] rounded-full" /></div></motion.div>}
                  </AnimatePresence>
                )}
                <div ref={messagesEndRef} className="h-4" />
              </div>

              <div className="p-4 bg-[#1a1f1d] border-t border-[#232a28] z-20 transition-colors">
                <form onSubmit={handleSendMessage} className="flex items-end gap-3 max-w-5xl mx-auto w-full">
                  <div className="flex items-center gap-1 pb-1.5">
                    <button type="button" onClick={() => showToast("Attachments coming soon", "info")} className="p-2 text-gray-400 hover:text-white hover:bg-[#232a28] rounded-lg transition-colors"><Paperclip size={20}/></button>
                    <button type="button" onClick={() => showToast("Emoji picker coming soon", "info")} className="p-2 text-gray-400 hover:text-white hover:bg-[#232a28] rounded-lg transition-colors"><Smile size={20}/></button>
                    <button type="button" onClick={() => showToast("Voice notes coming soon", "info")} className="p-2 text-gray-400 hover:text-white hover:bg-[#232a28] rounded-lg transition-colors"><Mic size={20}/></button>
                  </div>
                  <textarea ref={textareaRef} value={newMessage} onChange={handleInputChange} maxLength={5000} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendMessage(e); } }} placeholder="Type a message..." className="flex-1 bg-[#151817] text-white border border-[#323d38] focus:border-[#0fa384] rounded-xl px-4 py-3 placeholder-gray-500 text-[15px] resize-none custom-scrollbar outline-none transition-all focus:shadow-[0_0_10px_rgba(15,163,132,0.15)]" rows="1" />
